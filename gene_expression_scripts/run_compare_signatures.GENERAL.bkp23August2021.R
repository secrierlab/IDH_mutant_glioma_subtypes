...
for(i in 1:nrow(sampleinfo)){
 ...
 # annotation_with_scores<-merge(annotation_all,scores_hs_lgg,by.x="Samples",by.y="ID")
  ...
  dev.off()
	...
	annotation_all_compare_genes<-ann3[,c("Samples","type","IDH1_R132H","IDH1_other","TP53","ATRX","IDH_codel_subtype","chr7_gain_chr_10loss","histology","clusters")]
	annotation_all_compare_genes$status2<-rep(NA,nrow(annotation_all_compare_genes))
	annotation_all_compare_genes$status2[-which((annotation_all_compare_genes$IDH1_R132H==1 | annotation_all_compare_genes$IDH1_other==1) & annotation_all_compare_genes$TP53==1 & annotation_all_compare_genes$ATRX==1)]<-0
	annotation_all_compare_genes$status2[which((annotation_all_compare_genes$IDH1_R132H==1 | annotation_all_compare_genes$IDH1_other==1) & annotation_all_compare_genes$TP53==1 & annotation_all_compare_genes$ATRX==1)]<-1
  ...
  #
  # get only the samples with mutation in IDH1
  #
  ann_idh1<-ann3[which(ann3$IDH1==1),]
  
  matrix_onlyIDH1<-matrix_to_plot_hs_nocombat2[,which(colnames(matrix_to_plot_hs_nocombat2)%in%ann_idh1[,1])]

  #
  # get only the samples with mutation in IDH1 and LGG
  ann_idh1_lgg<-ann3[ann3$type=="LGG" & (ann3$IDH1 ==1),]
  
  matrix_onlyIDH1_lgg<-matrix_to_plot_hs_nocombat2[,which(colnames(matrix_to_plot_hs_nocombat2)%in%ann_idh1_lgg[,1])]
...
# 	write.table(ortholog_table_res2,file=output_txt2_var,sep="\t",row.names=T,quote=F)
	...
	plot_heatmap_hs(matrix_to_plot_fs,annotation_all2,output_hs=output_hs,output_hs2=output_hs_var,column_title="DEGs mm in GBM and LGG,no combat",width=10,split=TRUE,km=FALSE,number_clusters_row=3,how_split="mod2_classification",strong_cluster=T,png_output=F)
...
#   plot_heatmap_hs(matrix_to_plot_fs,annotation_with_scores,output_hs=output_hs,output_hs2=output_hs_var,column_title="DEGs mm in GBM and LGG,no combat",width=10,split=TRUE,km=FALSE,number_clusters_row=3,how_split=mod2_classification,strong_cluster=T)
  ...
  matrix_to_plot_fs_lgg<-matrix_to_plot_fs[,which(colnames(matrix_to_plot_fs)%in%annotation_fs_lgg[,1])]
	...
	ann_temp<-ann3[,c("Samples","type","IDH1_R132H","IDH1_other","TP53","ATRX","IDH_codel_subtype","chr7_gain_chr_10loss","histology","clusters")]
	ann_temp$status2<-rep(NA,nrow(ann_temp))
	ann_temp$status2[-which((ann_temp$IDH1_R132H==1 | ann_temp$IDH1_other==1) & ann_temp$TP53==1 & ann_temp$ATRX==1)]<-0
	ann_temp$status2[which((ann_temp$IDH1_R132H==1 | ann_temp$IDH1_other==1) & ann_temp$TP53==1 & ann_temp$ATRX==1)]<-1
  ...
  output_xlsx<-paste(paste(output_dir,output_degs,sep="/"))
	...
	ann_meth<- ann_meth[,c("Samples","type","IDH1_R132H","IDH1_other","TP53","ATRX","IDH_codel_subtype","chr7_gain_chr_10loss","histology","clusters","IDH_methylation_clusters","Supervised_DNA_methylation_clusters")]
