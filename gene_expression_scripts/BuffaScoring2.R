...
for(i in 1:nrow(sampleinfo)){
  ...
  for(ib in 1:length(matrices_for_gsva)){
    ...
    for(scorb in 1:length(gene_sets_list)){
      ...
      scores_current_gs<-apply(do.call(rbind,scores_by_gs),2,table)
      #Compute percentages of genes that are UP or Down in each sample
      scores_current_gs2<-apply(scores_current_gs,2,FUN=function(X){X/sum(X)})
      scores_current_gs3<-apply(scores_current_gs,2,FUN=function(X){X[2]/X[1]})
      
      names(scores_current_gs3)<-colnames(temp_matrix_gs)
      
      results_scoring[[scorb]]<-scores_current_gs3
    ...
    pdf(paste(current_group,"_Buffa_scoring.NORM.pdf",sep=""),width=12)
