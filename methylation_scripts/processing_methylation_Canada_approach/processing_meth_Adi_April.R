...
for(i in 1:length(list_meth_file)){
  ...
  for(row_ann in 1:nrow(df_ann)){
    ...
    genes<-df_ann[row_ann,ncol(df_ann)-1]
...
save(res_meth_wide,file="methylation_levels_DMRs_Adi_proc_Canada.RData")

pdf("weighthed_mean_DMRs_Adi.pdf")
...
pdf("weighthed_mean_dmrs_hist.regions.pdf")
