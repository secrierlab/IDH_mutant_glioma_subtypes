...
ann_regions<-read.delim(file="annotation_dmrs_adi_pval0.05_lfc1.txt")
promoters_genes<-ann_regions[ann_regions$annot.type%in%"mm10_genes_promoters","annot.symbol"]
...
png("mm_heatmap_regions_dmrs_adi_pval0.05_lfc1.newestimation.December.png", height=8,width=4,units='in',res=300)

#hsmatrix<-as.matrix(res_meth3[,-1])
#fx<-function(x){(x-mean(x))/(max(x)-min(x))}
#hsmatrix<-t(apply(hsmatrix,1,fx))
hsmatrix<-res_meth3[,-1]
...
hsmap<-Heatmap(hsmatrix,
	       ...
	       col=circlize::colorRamp2(c(0,0.25,0.5,0.75,1), c("Darkblue","orange2","orange","red","red2")))
draw(hsmap)

dev.off()

png("mm_heatmap_regions_dmrs_adi_pval0.05_lfc1.newestimation.Promoters.December.png", height=8,width=4,units='in',res=300)

#hsmatrix<-as.matrix(res_meth3[,-1])
#fx<-function(x){(x-mean(x))/(max(x)-min(x))}
#hsmatrix<-t(apply(hsmatrix,1,fx))
hsmatrix<-res_meth3[res_meth3[,1]%in%promoters_genes,-1]
fx<-function(x){(x-mean(x))/(max(x)-min(x))}
hsmatrix<-t(apply(hsmatrix,1,fx))
...
png("mm_heatmap_regions_dmrs_adi_pval0.05_lfc1.newestimation_no_redo_no_control.December.png", height=8,width=4,units='in',res=300)
...
hsmatrix<-res_meth3[res_meth3[,1]%in%promoters_genes,columns_to_save]
hsmatrix[is.na(hsmatrix)]<-0 #there are few values with NA

png("mm_heatmap_regions_dmrs_adi_pval0.05_lfc1.newestimation_no_redo_no_control.Promoters.December.png", height=8,width=4,units='in',res=300)
...
png("mm_heatmap_regions_dmrs_adi_pval0.05_lfc1.newestimation_no_redo_yes_control.December.png", height=8,width=4,units='in',res=300)
...
save(res_meth3,file="methylation_data_DMET_CpG_05_11_2020_pval0.05_lfc1_newestimation.December.RData")
