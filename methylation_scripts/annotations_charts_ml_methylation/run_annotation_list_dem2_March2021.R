...
save(res_meth3,file="methylation_data_DMET_CpG_30_03_2020_pval0.05_lfc1_newestimation.December.RData")
...
for(i in 1:length(list_meth_file)){
  ...
  for(row_ann in 1:nrow(df_ann)){
    ...
    test<-meth_table_current_mouse[which(meth_table_current_mouse[,1]==chrom & meth_table_current_mouse[,2]>=start & meth_table_current_mouse[,2]<=end),]
...
save(res_meth3,file="methylation_data_DMET_CpG_30_03_2020_pval0.05_lfc1_newestimation.December.Regions.RData")
...
pdf("beta_dmrs_hist.regions.pdf")
