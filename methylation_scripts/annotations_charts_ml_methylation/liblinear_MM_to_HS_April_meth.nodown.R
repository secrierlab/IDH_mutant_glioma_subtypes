...
plotPCA.simple<-function(input_for_ml3){
  
  data_pca <- input_for_ml3[,20:ncol(input_for_ml3)]
  data_pca[is.na(data_pca)]<-0
  pr <- prcomp(data_pca,  scale = TRUE)
  
  pr.pca<- data.frame(pr$x[,c(1:2)])
  
  pr.pca$sample<-input_for_ml3[,1]
  
  ann_pca<-input_for_ml3[,c(1:19)]
  
  pr.pca.simple<-merge(pr.pca,ann_pca,by.x="sample",by.y="Case")
  
  return(pr.pca.simple)
}

pdf("PCA_directlyOnDMRs.pdf",w=9)

pr.pca.simple <- plotPCA.simple(input_for_ml3)

pr.pca.simple$DNA_methylation_clusters<-gsub(pr.pca.simple$DNA_methylation_clusters,pattern="no_ann",replacement="LGr_Unkown")

p.pca <- ggplot(data = pr.pca.simple, aes(x = PC1, 
                                          y = PC2,colour=DNA_methylation_clusters)) + 
  geom_point(aes(colours=DNA_methylation_clusters))+scale_shape_manual(values = c(15,16,17,18,19,0,1))+ scale_color_manual(breaks = c("LGm1", "LGm2", "LGm3","LGm4","LGm5","LGm6","LGr_Unkown"),values=c("brown2", "darkorchid3", "dodgerblue4","olivedrab3","olivedrab3","olivedrab3","black")) + geom_hline(yintercept=0, linetype="dashed", color = "black") + geom_vline(xintercept = 0, linetype="dashed", color = "black")+ggtitle(paste("n.samples:",sum(nsamples_tests_sets)))	

print(p.pca)

dev.off()

pdf("PCA_directlyOnDMRs_LGGonly.pdf",w=9)

pr.pca.simple <- plotPCA.simple(input_for_ml3[which(input_for_ml3$type=="LGG"),])
...
current_lgg<-list_lgg[[1]]
current_measure<-names(list_lgg)[1]
current_lgg<-current_lgg[-which(current_lgg[,2]%in%"X0"),]
current_lgg[,2]<-gsub(current_lgg[,2],pattern="X1",replacement="IDH-yes")
...
pdf("LGGpredictions_NODOWNSAMPLING.pdf")
ggplot(current_lgg, aes(x=variable, y=prob))+geom_boxplot()+ggtitle(nsamples)+ylab("Probability")+
  geom_jitter(shape=16, position=position_jitter(0.2),aes(color = meth_levels))+scale_colour_gradient(high="#CE2D4F",low="#28587B")+theme_bw()+
...
current_gbm_lgg<-list_gbm_lgg[[1]]
current_measure<-names(list_gbm_lgg)[1]
current_gbm_lgg<-current_lgg[-which(current_gbm_lgg[,2]%in%"X0"),]
current_gbm_lgg[,2]<-gsub(current_gbm_lgg[,2],pattern="X1",replacement="IDH-yes")
current_gbm_lgg$Classif <- sapply (current_gbm_lgg$prob,
                               function(x) ifelse(x>=0.5,"yes","no"))
t <- table(current_gbm_lgg[,c("variable","Classif")])
t.frac <- t[,2]/(t[,2]+t[,1])
df.frac <- data.frame(variable=names(t.frac),Fraction=round(t.frac,2))

pdf("GBM-LGGpredictions_NODOWNSAMPLING.pdf")
ggplot(current_gbm_lgg, aes(x=variable, y=prob))+geom_boxplot()+ggtitle(nsamples)+ylab("Probability")+
  geom_jitter(shape=16, position=position_jitter(0.2),aes(color = meth_levels))+scale_colour_gradient(high="#CE2D4F",low="#28587B")+theme_bw()+
  geom_text(data = df.frac,
            aes(variable, Inf, label = Fraction), vjust = 1)+
  geom_hline(yintercept = 0.5, linetype="dotted", 
...
for(cira in 1:10000){
	...
	if (cira%%10==0) print(paste("Iteration:",cira))
