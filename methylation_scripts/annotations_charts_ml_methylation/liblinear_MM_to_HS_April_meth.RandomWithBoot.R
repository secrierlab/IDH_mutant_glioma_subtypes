...
for(i in 1:100){
	...
	lgg_ml_res<-libLinear_mouse_hsAprilRANDOM(input_ml5_new,input_mm2,list_ann=list_ann,type_tumours="LGG",prefix_output="LGG",suffix_output="DMRs_hyper_hypo")
	gbm_lgg_ml_res<-libLinear_mouse_hsAprilRANDOM(input_ml5_new,input_mm2,list_ann=list_ann,type_tumours=c("GBM","LGG"),prefix_output="GBM_LGG",suffix_output="DMRs_hyper_hypo")
	...
	input_ml5_new_temp<-input_ml5_new[input_ml5_new$type%in%"LGG",]
	  
	input_pca<-input_ml5_new_temp[,19:ncol(input_ml5_new_temp)]
	input_pca[is.na(input_pca)]<-0
	res.pca <- prcomp(input_pca[,-1], scale = TRUE)
	
	pdf(paste(paste("PCA_BOOT_RANDOMGENES_iter",i,ngenes_dmr_mouse,sep="."),".pdf",sep=""))
...
save(list=c("lgg_melt","gbm_lgg_melt"),file="liblinear_randomgenes_methylationWithBoot.RData")

pdf("boxplot_stats_liblinear_randomgenesWithBoot.pdf")
